...
const LandingPage = () => {
  ...
  // New state for course suggestions
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      const data = await api.getCourseSuggestions();
    ...
    <>
      <Box sx={{ bgcolor: 'background.default', color: 'text.primary' }}>
        ...
        <Box sx={{ py: 8, bgcolor: 'background.paper' }}>
          <Container maxWidth="lg">
            <Typography variant="h4" component="h2" gutterBottom textAlign="center" fontWeight="bold">
              Propón y Vota por Nuevos Cursos
            </Typography>
            <Typography variant="subtitle1" color="text.secondary" textAlign="center" sx={{ mb: 4 }}>
              ¿No encuentras el curso que buscas? ¡Propón uno nuevo o vota por tus ideas favoritas!
            </Typography>

            <CourseSuggestionForm onNewSuggestion={fetchSuggestions} onAuthRequired={handleOpenAuthModal} />

            <Box sx={{ mt: 6 }}>
              <Typography variant="h5" component="h3" gutterBottom textAlign="center" fontWeight="bold" sx={{ mb: 3 }}>
                Sugerencias de Cursos Populares
              </Typography>
              {loadingSuggestions ? (
                <Typography textAlign="center">Cargando sugerencias...</Typography>
              ) : (
                <Grid container spacing={3}>
                  {courseSuggestions.length > 0 ? (
                    courseSuggestions.map(suggestion => (
                      <Grid item xs={12} sm={6} md={4} key={suggestion.id}>
                        <CourseSuggestionCard suggestion={suggestion} onVoteSuccess={fetchSuggestions} onAuthRequired={handleOpenAuthModal} />
                      </Grid>
                    ))
                  ) : (
                    <Typography textAlign="center" sx={{ width: '100%', mt: 2 }}>
                      No hay sugerencias de cursos aún. ¡Sé el primero en proponer una!
                    </Typography>
                  )}
                </Grid>
              )}
